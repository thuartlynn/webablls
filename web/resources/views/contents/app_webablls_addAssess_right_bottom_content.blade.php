<div class="">
  <hr>
  <p class="app-right-bottom-title">Student Age (years):</p>
  <p class="mt-n2"><?php echo $NewAssInfo["Age"]; ?></p>
  
    <form action="/Student/AddAssess/<?php echo $NewAssInfo["ID"];?>" class="app-inline needs-validation" id="addAssess" method="post" novalidate autocomplete="off">{{ csrf_field() }}
      ...
      ?>
      <input id="datepicker" class="app-right-bottom-input form-control app-option-font app-right-bottom-content-mr" name="assessDate" type="text" required />
      <div class="valid-feedback">Valid.</div>
      <div class="invalid-feedback" id="invalid-date">Please fill out this field.</div>
      <br>
      ...
      <ul class="nav nav-tabs p-0 mt-4" role="tablist" >
        <li class="app-right-bottom-title">
            Palette 1
        </li>
      ...
      <div class="tab-content p-3">
        <div id="palette1" class="container tab-pane p-0 mt-n4 active"><br>
          ...
          <?php
            ...
            $UsedColor = $NewAssInfo["UsedColor"];
            ...
            echo "<div class='test'>";
              for($i=0; $i<=7; $i++){
                ...
                if ($UsedColor->contains($colorpalette[$i])) {
                  ...
                  //echo "沒有使用";
                  echo "
                        <input type='radio' id='color$i' class='hide form-check-input' name='color' value='$colorpalette[$i]' onclick='chooseColor(this)' />
      ...
      <div>
        <p class="app-right-bottom-title">Chosed Color</p>
          <div class="ml-1" style="" id="chooseColor"></div><p id="text" class="d-inline-block"></p>
          
        <p class="app-right-bottom-title">Used Colors</p>
          <?php
            if (count($NewAssInfo["UsedColor"]) == 0) {
              echo '';
            } else {
              foreach($NewAssInfo["UsedColor"] as $value) {
      ...
      <div class="">
        ...
        <textarea name="Notes" rows="4" class="notes"></textarea>
      </div>

      <hr>
      <div class="mt-4">
        <h5 class="app-nopadding-nomargin text-secondary">Program Level During the Time of Assessment</h5>
      </div>
      <br>
      ...
      <br>
      <div id="method">
        <h5 class="app-nopadding-nomargin mt-2 text-secondary">Edit Method</h5>
        <?php 
          if (Auth::user()->assessment_editmode == 1) {
            echo '<input type="radio" id="Text" name="method" value="text" checked="true">
                  <label for="Text" class="mr-2">Text</label>
    
                  <input type="radio" id="Grid" name="method" value="grid">
                  <label for="Grid" class="mr-2">Grid</label>
          
                  <input type="radio" id="Cat" name="method" value="cat">
                  <label for="Cat">Cat</label>';
          } else if (Auth::user()->assessment_editmode == 0) {
            echo '<input type="radio" id="Text" name="method" value="text">
                  <label for="Text" class="mr-2">Text</label>
          
                  <input type="radio" id="Grid" name="method" value="grid" checked="true">
                  <label for="Grid" class="mr-2">Grid</label>
            ...
            echo '<input type="radio" id="Text" name="method" value="text">
                  ...
                  <input type="radio" id="Cat" name="method" value="cat" checked="true">
                  <label for="Cat">Cat</label>';
          }
        ?>
      </div>
      ...
      <br>
      <input type="submit" value="Continue" name="Continue" class="btn btn-sm btn-secondary app-right-bottom-mr" />
    </form>
</div>
